...
class DataProcessing(object):
    def __init__(self, test_path='测试A.xlsx'):
        ...
        print("shape of train", self.train_X.shape)
    ...
    def TestDataFormalized(self):
        test_data = self.test_data.values
        test_data = np.delete(test_data, self.firstdelete, axis=1)
        nullmatrix = self.test_data.isnull().values[:, 1:]
        tmpn = np.zeros(nullmatrix.shape)
        tmpn[nullmatrix] = 1
        nullmatrix = nullmatrix==0
        self.test_id = test_data[:, 0]
        self.test_X = test_data[:, 1:]
        print("test shape", self.test_X.shape)
        print("methods", len(self.methods))
        #testtypes = self.GetColumnTypes(self.test_X, nullmatrix)
        #print(np.sum(np.array(self.types)==np.array(testtypes)))
        stridxs = []
        for i in range(len(self.methods)):
            data = self.test_X[:, i]
            nn = nullmatrix[:, i]
            rn = BoolReverse(nn)
            if np.sum(rn) < nullmatrix.shape[0]:
                data[nn] = self.mfs[i]
            if i in self.str2num:
                stridxs.append(i)
                one_hot = OneHot_Str(data, s2nd=self.str2num[i])
                self.test_X = np.concatenate((self.test_X, one_hot), axis=1)
            else:
        ...
        for i in range(self.num_column):
            mean = self.x_pca_norm[i][0]
            std = self.x_pca_norm[i][1]
            data = self.test_X[:, i]
            data = (data - mean) / std
            self.test_X[:, i] = data
        print("test data transferred over")
